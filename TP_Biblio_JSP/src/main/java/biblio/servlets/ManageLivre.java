	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if ((idlivre == null) || (idlivre.trim().length() <= 0)) {
			...
			if ((titre == null) || (titre.trim().length() <= 0)) {
				ctrl = false;
				messageErreur = messageErreur + "Veuillez saisir le titre<br/>";
			}
			if ((nbpages == null) || (nbpages.trim().length() <= 0)) {
				ctrl = false;
				messageErreur = messageErreur + "Veuillez saisir le nombre de pages<br/>";
			}
			try {
				...
				e.printStackTrace();
				ctrl = false;
				messageErreur = messageErreur + "Le nombre de pages n'est pas une valeur correcte<br/>";
			}
            if (ctrl) {
            	try {
			...
			if ((titre == null) || (titre.trim().length() <= 0)) {
				ctrl = false;
				messageErreur = messageErreur + "Le titre doit être renseigné !<br/>";
			}
			if ((nbpages == null) || (nbpages.trim().length() <= 0)) {
				ctrl = false;
				messageErreur = messageErreur + "Le nombre de pages doit être renseigné !<br/>";
			}
			try {
				nbpa = Integer.parseInt(nbpages);
			} catch (Exception e) {	
